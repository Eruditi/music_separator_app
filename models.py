...
class ModelNotFoundError(Exception):
    """模型未找到异常"""
    def __init__(self, message: str, model_type: Optional[str] = None, download_url: Optional[str] = None):
...
class BaseModel:
    ...
    def separate(self, audio_path: str, output_dir: str, progress_callback: Optional[Callable[[int, int, str], None]] = None) -> List[str]:
        """分离音频
...
class DemucsModel(BaseModel):
    ...
    def _parse_model_type(self, model_type: str) -> Tuple[str, int]:
        """解析模型类型字符串

        Args:
            model_type: 模型类型字符串，如 "demucs:2stems"

        Returns:
            Tuple[str, int]: (模型名称, 轨道数)
        """
    ...
    def separate(self, audio_path: str, output_dir: str, progress_callback: Optional[Callable[[int, int, str], None]] = None) -> List[str]:
        ...
        def report_progress(step: int, total: int, description: str) -> None:
            """报告进度"""
        ...
        try:
            ...
            # 使用配置参数
            shifts: int = getattr(self, 'shifts', 1)
            overlap: float = getattr(self, 'overlap', 0.25)

            sources: Any = apply_model(
                self.model,
                wav,
                device=device,
                shifts=shifts,
    ...
    def _save_2stems(self, sources: List[Any], base_name: str, output_dir: str) -> List[str]:
        """保存2轨分离结果（人声+伴奏）
    ...
    def _save_multistem(self, sources: List[Any], base_name: str, output_dir: str) -> List[str]:
        """保存多轨分离结果
